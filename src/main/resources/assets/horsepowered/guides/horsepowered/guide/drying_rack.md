...
The Drying Rack is a passive wooden rack that slowly converts wet or raw items into a dried form. It holds **8 items at once** in a 4×2 grid laid across the slats, and each slot ticks its own progress timer independently, so you can load the rack one slot at a time without resetting anyone else's progress.
...
- Modpack makers can add custom drying recipes at `data/<namespace>/recipe/drying/<recipe>.json`. See the README for the recipe schema.
