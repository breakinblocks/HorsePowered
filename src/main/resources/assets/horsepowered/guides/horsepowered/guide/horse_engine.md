...
Pack makers can set a fixed jump strength for any entity type with the `horsepowered:horse_engine_jump_strength` data map. Place a file at `data/<namespace>/data_maps/entity_type/horse_engine_jump_strength.json` in a datapack:
...
A listed value replaces the mob's own attribute. The value is read when the worker is attached, so re-attach a worker after changing the file.
