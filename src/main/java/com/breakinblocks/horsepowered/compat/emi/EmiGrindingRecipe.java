...
public class EmiGrindingRecipe extends BasicEmiRecipe {
    ...
    public EmiGrindingRecipe(EmiRecipeCategory category, GrindstoneRecipe recipe) {
        super(category, recipe.getId(), widthFor(recipe), heightFor(category, recipe));
        this.time = recipe.getTime();
        this.secondary = recipe.getSecondary();
        this.secondaryChance = recipe.getSecondaryChance();
        this.manual = category == HorsePoweredEmiPlugin.GRINDING_HAND;
    ...
    private static int heightFor(EmiRecipeCategory category, GrindstoneRecipe recipe) {
        return category == HorsePoweredEmiPlugin.GRINDING_HAND && recipe.getHungerCost() > 0.0F ? 38 : 28;
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        if (manual) {
            ...
            widgets.addText(
                    Component.translatable("gui.horsepowered.jei.turns", turns),
                    0, 20, 0x808080, false);
        } else {
            widgets.addText(
                    Component.translatable("gui.horsepowered.jei.time", time),
                    0, 20, 0x808080, false);
        }
