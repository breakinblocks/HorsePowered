...
- `HPBlockEntityHorseBase.validateArea()` now also recomputes `pathSpeedMultiplier` and calls `setChanged()` when either validity or the multiplier changes. The multiplier is saved via the new `ValueOutput`/`ValueInput` API and sent in the BE update packet so client-side worker animations stay aligned with server movement.
- `HorsePowerConfig` gained a `[horse_path]` config section plus a small `Identifier` map parser. Config reloads invalidate the cached map via `ModConfigEvent.Loading` / `Reloading`.
