...
Hotfix port of the 1.21.1 branch's 2.5.1 release into 1.20.1.
...
- **Horse Power Generator stone-tier requirement and charge persistence**: the generator now sits in both the pickaxe and `needs_stone_tool` tags and ships with a self-drop loot table. Stored FE is attached to the dropped item under `BlockEntityTag`, and placing the item back restores the buffer. The horse, if any, is still released as a live entity with a lead, so charge moves with the block while the worker is freed.
...
- `HPBlockEntityHorseBase.validateArea()` now also recomputes `pathSpeedMultiplier` and calls `setChanged()` when either validity or the multiplier changes. The multiplier is saved to NBT and sent in the BE update packet so client-side worker animations stay aligned with server movement.
- `Configs` gained a `[horse_path]` config section plus a small `ResourceLocation` map parser. Config reloads invalidate the cached map via `ModConfigEvent.Loading` / `Reloading`.
- The hand grindstone's `use()` only turns the wheel when the player holds nothing in the active hand, matching the empty-hand discipline the newer branches enforce via the split `useWithoutItem` / `useItemOn` handlers.
- Flint and Tinder lighting falls back to vanilla 1.20.1's campfire/candle/TNT checks instead of the `FIRESTARTER_LIGHT` tool action, which does not exist in Forge 47.x.
